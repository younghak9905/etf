...
- Clarified Cloud Run UTC timestamps by adding configured-timezone display fields to run summaries and root-page Last Run output; Discord test/signal messages now show observed times in `APP_TIMEZONE`. Verified with `C:\Users\yeong\AppData\Local\Programs\Python\Python311\python.exe -m unittest discover -s tests`.
- Added last-run market snapshot persistence and root-page display for registered instruments, including current price, day high/low, previous close, volume, observed time, and quote source. Verified with `C:\Users\yeong\AppData\Local\Programs\Python\Python311\python.exe -m unittest discover -s tests`.
